...
public class ElasticsearchDataGenerator {
    public static void main(String[] args) throws IOException {
        ...
        List<News> newsFromMySQL = getNewsFromDB(sqlSessionFactory);
        for (int i = 0; i < 10; i++) {
        ...
        try (RestHighLevelClient client = new RestHighLevelClient(RestClient.builder(new HttpHost("localhost", 9200, "http")))) {
            for (int i = 0; i < 1000; i++) {
                ...
                for (News news : newsFromMySQL) {
                    ...
                    data.put("content", (news.getContent().length() - 10) > 0
                            ? news.getContent().substring(0, 10) : news.getContent());
    ...
    private static List<News> getNewsFromDB(SqlSessionFactory sqlSessionFactory) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            return session.selectList("com.github.tdd.MockMapper.selectNews");
        }
    }
}
